...
nodeAVL * FindAVL(int key ,nodeAVL*root , AVL* T){
    if(root == NULL){
        (T->comp)++;
        return NULL;

    }
   // printf("the value of key is %d, the value of root key is %d\n",key,root->key);
    if(key<root->key){
        (T->comp)++;
...
void AVLinsertNode(int key , AVL* T){
    //printf("the node is getting Inserted here!!\n");
